class StatsController < ActionController::Base
  ...
  def get_players
    respond_to do |f|
      ...
      f.json { render json: Player.all.map {|p| {value: p.name, tokens: p.name.split(' '), id: p.id, slug: p.slug, pic_url: p.profile_pic_thumb_url, number: p.last_number, team: p.last_team.try(:name)}} }
    end
  end
